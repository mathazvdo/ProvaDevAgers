...
public class ExamesFuncionarioDao extends Dao {
	...
	public List<ExameFuncionarioVo> todosExames(){
		List<ExameFuncionarioVo> listaDeExames = new ArrayList<ExameFuncionarioVo>(); // criando uma lista de exames
		...
		try (Connection conn = getConexao();PreparedStatement ps = conn.prepareStatement(sql);
			...
			while (rs.next()) {
				ExameFuncionarioVo exameFuncionarioVo = new ExameFuncionarioVo(); //extraindo uma variavel local
		...
		return listaDeExames; // devolvendo a lista de exames
	}
	...
	public ExameFuncionarioVo buscarExameFuncionarioPorId(String id_funcionario, String id_exame, Date dataExame) {
	    String sql = "SELECT f.rowid AS id_funcionario, e.rowid AS id_exame, ef.dataExame "
	            + "FROM exameFuncionario ef "
	            + "JOIN funcionario f ON ef.id_funcionario = f.rowid "
	            + "JOIN exame e ON ef.id_exame = e.rowid "
	            + "WHERE ef.id_funcionario = ? AND ef.id_exame = ? AND ef.dataExame = ?";
	    
	    try (Connection conn = getConexao(); PreparedStatement ps = conn.prepareStatement(sql)) {
	        ps.setString(1, id_funcionario);
	        ps.setString(2, id_exame);
	        ps.setDate(3, new java.sql.Date(dataExame.getTime()));
	    ...
	    return null; // Se não encontrar o exame
	}
	...
	public List<ExameFuncionarioVo> todosExamesPorIdFuncionario(String rowid) {
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		String query = "SELECT id_funcionario, id_exame, dataExame from exameFuncionario WHERE id_funcionario = ?";
